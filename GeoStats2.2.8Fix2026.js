...
(function() {
    ...
    // Install interceptor IMMEDIATELY before any other scripts run
    ...
    window.fetch = function(...args) {
        ...
        if (url && url.includes('/guess')) {
            const urlMatch = url.match(/geoguessr\.com\/.*?\/([a-f0-9]+)\/guess/);

            if (urlMatch && urlMatch[1]) {
                const gameId = urlMatch[1];
                window.sessionStorage.setItem('current-duels-game-id', gameId);
                // console.log('Intercepted Game ID:', gameId);
            }
        }
...
function updateUserInfo() {
    if (USER_ID) return true; // Already found it
    try {
        const nextData = document.getElementById('__NEXT_DATA__');
        if (nextData) {
            const parsed = JSON.parse(nextData.innerText);
            USER_ID = parsed.props.accountProps.account.user.userId;
            // console.log("User info found:", USER_ID);
            return true;
        }
    } catch (e) {
        return false;
...
function duelsGameFinished(gameResults, player_index){
    // first evaluate every single round
    for(let round_number = 0; round_number < gameResults.rounds.length; round_number++) {
        let current_guess = [gameResults.teams[player_index].players[0].guesses[round_number].lat, gameResults.teams[player_index].players[0].guesses[round_number].lng];
        getGuessCountryCode(current_guess)
            .then((guess) => {
            //Get all values
            let location_code = gameResults.rounds[round_number].panorama.countryCode.toUpperCase();
            let distance = gameResults.teams[player_index].players[0].guesses[round_number].distance;
            let opponent_index = 0;
            if(player_index == 0){
                opponent_index = 1;
            }
            let userScore = gameResults.teams[player_index].roundResults[round_number].score;
            let opponentScore = gameResults.teams[opponent_index].roundResults[round_number].score;
            let startTime = gameResults.rounds[round_number].startTime;
            let timerTime = gameResults.rounds[round_number].timerStartTime;
            let guessTime = gameResults.teams[player_index].players[0].guesses[round_number].created;
            let opponentDistance = gameResults.teams[opponent_index].players[0].guesses[round_number].distance;
    ...
    if(gameResults.options.isRated){
        userRatingBefore = gameResults.teams[player_index].players[0].progressChange.rankedSystemProgress.ratingBefore;
        userRatingAfter = gameResults.teams[player_index].players[0].progressChange.rankedSystemProgress.ratingAfter;
    }

    let opponentRatingBefore = gameResults.teams[opponentIndex].players[0].rating;
    let userHealth = gameResults.teams[player_index].health;
    let opponentHealth = gameResults.teams[opponentIndex].health;
    let gameLength = gameResults.currentRoundNumber;
    let gameStartTime = gameResults.rounds[0].startTime;
    let gameEndTime = gameResults.rounds[gameLength-1].endTime;

    if(window.sessionStorage.getItem('current-duels-game-id') == gameResults.gameId){
        return;
    }
    window.sessionStorage.setItem('current-duels-game-id', null);
    ...
    let gameTime = timeToSeconds(gameEndTime) - timeToSeconds(gameStartTime);
    ...
    overallStats.opponentRating = ((overallStats.opponentRating * (overallStats.totalGames - 1)) + opponentRatingBefore) / overallStats.totalGames;
    ...
    if(userRatingBefore != null && userRatingAfter != null){
        ...
        if(win){
            ...
            overallStats.averageRatingChange.win[1] = ((overallStats.averageRatingChange.win[1] * (overallStats.averageRatingChange.win[0] - 1)) + (userRatingAfter-userRatingBefore)) / overallStats.averageRatingChange.win[0];
        }
        else if(!win){
            overallStats.averageRatingChange.loss[0]++;
            overallStats.averageRatingChange.loss[1] = ((overallStats.averageRatingChange.loss[1] * (overallStats.averageRatingChange.loss[0] - 1)) + (userRatingAfter-userRatingBefore)) / overallStats.averageRatingChange.loss[0];
        }
    }
    ...
    return new Promise((resolve,reject) => {
        fetch(`${api}?t=${Date.now()}`,{
            ...
            if(out.status == 'Finished'){
                duelsGameFinished(out, player_index);
                return;
            }
        })
            .catch((error) => {
...
function duelsCheck(){
    const gameId = window.sessionStorage.getItem('current-duels-game-id');
    const api_url = "https://game-server.geoguessr.com/api/duels/" + gameId;

    //If a round has finished
    //   if(document.getElementsByClassName('duels-game-wrapper_root__vIFY7').length > 0){
    //If the round is finished
    if(document.getElementsByClassName('result-map_roundPin__JvJuD').length > 0){
        if(!checkingDuelsRoundOver){
            checkingDuelsRoundOver = true;
            if (gameId == null) {
                console.warn('Dont send a request with gameID null');
            } else {
                setTimeout(() => {
    ...
    //If the game is finished
    else if(document.getElementsByClassName('post-game-progression-overlay_root__VfJDJ').length > 0){
...
function initGeostatsUI() {
    // Only run if the body exists
    if (!document.body) {
        setTimeout(initGeostatsUI, 50);
        return;
    }
...
// 3. Trigger UI initialization safely
if (document.readyState === 'loading') {
    // Wait until the initial HTML is parsed to avoid hydration errors
    window.addEventListener('DOMContentLoaded', initGeostatsUI);
} else {
    initGeostatsUI();
}
